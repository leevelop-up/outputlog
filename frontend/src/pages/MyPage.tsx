  ...
  return (
    <div className="mypage-wrap">
      ...
      <aside className="mypage-sidebar">
        <div className="mypage-character-card">
          <Character equippedItems={equippedItems} size={160} />
          <div className="mypage-level-badge" style={{ background: levelInfo.color }}>
            {levelInfo.icon} Lv.{levelInfo.level} {levelInfo.title}
          </div>

          {/* 경험치 바 */}
          <div className="mypage-xp-wrap">
            <div className="mypage-xp-bar">
              <div className="mypage-xp-fill" style={{ width: `${levelInfo.progress}%`, background: levelInfo.color }} />
            </div>
            <div className="mypage-xp-label">
              <span>{(user?.points ?? 0).toLocaleString()} pts</span>
              {levelInfo.nextPts && <span>다음: {levelInfo.nextPts.toLocaleString()} pts</span>}
            </div>
          </div>
          ...
          {badges.length > 0 && (
            <div className="mypage-badges">
              {badges.map(b => (
                <span key={b.label} className="mypage-badge" style={{ background: b.color }}>{b.label}</span>
              ))}
            </div>
          )}

          <Link to="/shop" className="btn btn-primary mypage-shop-btn">꾸미기 상점</Link>
        </div>
      </aside>
      ...
      <main className="mypage-main">
        ...
        <div className="auth-box" style={{ marginBottom: 24 }}>
              ...
              {editing ? (
                ...
                <>
                  ...
                  <div style={{ marginTop: 8, display: 'flex', gap: 12, flexWrap: 'wrap' }}>
                    {user?.githubUrl && (
                      <a href={user.githubUrl} target="_blank" rel="noopener noreferrer" style={{ fontSize: 12, color: 'var(--primary)', textDecoration: 'none' }}>
                        🐙 GitHub
                      </a>
                    )}
                    {user?.websiteUrl && (
                      <a href={user.websiteUrl} target="_blank" rel="noopener noreferrer" style={{ fontSize: 12, color: 'var(--primary)', textDecoration: 'none' }}>
                        🌐 홈페이지
                      </a>
                    )}
                  </div>
          ...
          {!editing && (
            <div style={{ marginTop: 14, padding: '10px 14px', borderRadius: 8, background: 'var(--bg-hover)', border: '1px solid var(--border)', fontSize: 12, color: 'var(--text-dim)', fontFamily: 'var(--mono)' }}>
              글 작성 +10pt · 글 삭제 -10pt · 상점에서 아이템 구매 가능
            </div>
          )}
        </div>
